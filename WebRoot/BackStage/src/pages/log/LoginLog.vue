    ...
    <el-card class="contain">
      <el-form :inline="true" :model="formSearch" class="search-form" size="mini">
        <el-form-item label="登录用户">
          <el-input v-model="formSearch.username" placeholder="登录用户" clearable></el-input>
        </el-form-item>
        <el-form-item label="登录时间" class="date-select">
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange" border style="width: 100%" :row-style="{'height':'40px'}" :cell-style="{'padding':'0'}"
        ...
        <el-table-column prop="system" label="登录系统" width="120">
        </el-table-column>
        <el-table-column prop="browser" label="浏览器" width="120">
        </el-table-column>
        <el-table-column label="操作" width="80">
          <template slot-scope="scope">
            <el-button type="danger" @click="onDelLoginLog(scope.row.id)" icon="el-icon-delete" size="mini"></el-button>
          </template>
        </el-table-column>
      </el-table>
  ...
  export default {
    ...
    methods: {
      ...
      requestData() {
        // 处理传时间段空值问题
        if (this.time != null) {
          this.formSearch.loginTimeFrom = this.time[0]
          this.formSearch.loginTimeTo = this.time[1]
        } else {
          this.formSearch.loginTimeFrom = null
          this.formSearch.loginTimeTo = null
        }
        fingLoginLogByPage(this.formSearch).then((res) => {
          if (res.status) {
